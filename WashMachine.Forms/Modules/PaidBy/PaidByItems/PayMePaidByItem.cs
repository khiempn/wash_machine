    ...
    public class PayMePaidByItem : IPaidByItem
    {
        ...
        private void RemoveRegisterEvents()
        {
            try
            {
                paymeService.CodeRecivedHandler -= PaymeService_CodeRecivedHandlerAsync;
                paymeService.PaymentLoopingHandler -= PaymeService_PaymentLoopingHandler;
                paymeService.RemoveRegisterEvents();
                        ...
                        if (paymentResponse.IsSuccess)
                        {
                            ...
                            if (paymentResponse.IsPaymentCompleted())
                            {
                                ...
                                paymentItem.PaymentCompletedCallBack.Invoke(mainForm, () =>
                                {
                                    ...
                                    paymentAlertUI.SetPaymeInvoice(orderRequest);
                                    paymentAlertUI.Show();
                                });
                            }
                            else
        ...
        public async void Click()
        {
            try
            {
                ...
                if (_payment == null)
                {
                    waitingUI.SetErrorMessage("Unable to create a payment for Payme. Please try again.");
                    waitingUI.StartTimerCloseAlert();
                    Program.octopusService.SetUserIsUsingApp(false);
                    return;
                }
